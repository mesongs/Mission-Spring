...
public class OcrController2 {
	...
	@RequestMapping("/ocr/list")
	public String generalOcr() {

		// 비동기(execute) or 동기(enqueue) 요청을 편리하게 할 수 있음
		// HttpURLConnection과 HttpClient에 비해 상당히 직관적이고 사용하기도 편리
		// HttpURLConnection은 매번 통신을 할 때마다 openConnection(), disconnect() 열고 닫기 해줘야함
...
//		  		.addHeader("postman-token", "31917011-c262-8273-4761-7a87eef0d3fa")
	...
	// general OCR 요청 이거로 해야함
	// 실제 api 호출 test ('합'으로 시작하는 inferText의 index or '계'를 포함하는 index or '금액'을 포함하는 index)
	// java에서 api요청하는 방법 restTemplate도 존재함
	// 합계, 소계, 결제금액, 판매금액 => List의 최대값 추출
	@RequestMapping("/ocr/test2")
	@org.springframework.web.bind.annotation.ResponseBody
	public List<String> testOcr2() {
		...
		// 쉼표를 공백으로 대체한 실제 합계금액이 저장되는 리스트
		List<Integer> resultList = new ArrayList<Integer>();
		StringBuilder sb = new StringBuilder(); // 결과 문자값 저장
...
//		  		.addHeader("postman-token", "31917011-c262-8273-4761-7a87eef0d3fa")
		...
		try {
			...
			String result = responseBody.string();
			System.out.println("api로 받아온 general OCR : " + result);
			// 세진이는 본인이 만든 api에서 값을 받아와서 바로 map으로 넣음
			...
			System.out.println("correctIndexList : " + correctIndexList);
			// [41, 48, 49]
...
//			System.out.println(inferTextList.get(49));
		...
		// @ResponseBody List 출력
		return inferTextList;

	}

	@GetMapping("/ocr/enter")
	public String getOcrPage() {
...
//			System.out.println("List 출력 : " + inferTextList);
			...
			// 리스트 하나씩 돌면서, '합'으로 시작하는 글자가 있는 인덱스를 추출
			for (int i = 0; i < inferTextList.size(); i++) {
			...
			// correctIndexList에는 [24,66]
			for (int i = 0; i < correctIndexList.size(); i++) {

				// 0번 인덱스에 24번 저장됨, 25번부터 int형으로 변환할 수 있는 숫자를 찾아야함
				int cnt = correctIndexList.get(i);

				sb.append(inferTextList.get(cnt));
				// sb에는 inferTextList.get(24) = "합계" 저장
				// 25번 인덱스부터 int형으로 변환할 수 있는 글자를 찾는다(쉼표를 공백으로 대체한), 여기서는 26번 인덱스에서 4,100 => 4100
				// int형 변환 가능 => resultList

				// inferTextList의 size까지만 동작해야함 cnt < inferTextList.size() -1
				// 66 < 102 => 101까지만 돌아야함, 인덱스는 0~102, 66<103(size())까지돌면, 102 + 1 => 103되서
				// 존재하지 않는 [103] 인덱스를 참조하게됨
				// outofBounds 에러는 int값 체크하는 부분에서 발생함 =>

				while (cnt < inferTextList.size() - 1) {

					// inferTextList의 25번 인덱스부터 int형으로 변환 가능한 값 찾기
					cnt = cnt + 1;

//					inferTextList.get(cnt).replaceAll(",", "");
					if (checkInt(inferTextList.get(cnt).replaceAll(",", ""))) {
...
//			inferTextList.get(24);
//			inferTextList.get(66);

			// inferTextList의 인덱스 24, 66 => temp에 저장

			// 합이라는 글자로 시작하는 인덱스 추출 [24,66]
//			System.out.println(inferTextList.get(24));
